...
形狀對不上、想自己擺？還有一塊**自由畫板**（`#/board`）：拖形狀、連線、改大小顏色，
**線會黏在形狀上**。填表產生的圖可以整張搬進畫板繼續微調。
...
**但畫板的定位是「程式先排好，你再微調」，不是「從空白開始畫」。**
從空白開始畫流程圖，跟在 Word 裡拉方塊一樣痛苦——那正是這個站要解決的問題。
所以流程圖的填表畫面有一顆「把這張圖搬到畫板繼續改」，搬過去之後每一格都拖得動。
...
| 指令 | 內容 |
|---|---|
| `npm test` | 全部：`core.unit`（156）＋`gen.unit`（63）＋`board.unit`（33）＋`data`（25）＋`e2e`（110），約 4 分鐘 |

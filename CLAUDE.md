...
`core.js` 的 `buildDocx()`。圖同時放 SVG 與 PNG 兩份：

- Word 2016／Microsoft 365 讀 SVG（`a:blip` 底下的 `asvg:svgBlip` 擴充），
  使用者按右鍵「轉換成圖形」就把整張圖變成 Word 圖案群組。**這才是這個功能存在的理由**，
  貼一張 PNG 進去誰都會。
- 舊版 Word 與 LibreOffice 讀那份 PNG。**兩份都要放**，只放 SVG 會變成一個空白框，
  而且畫面上完全看不出哪裡錯了。
- ZIP 只做 store（不壓縮）。docx 允許，而且省掉一整包 deflate——零相依是鐵律。
  時間固定成 1980-01-01，同樣的內容壓出同樣的位元組，測得起來。
- **CI 驗不了 Word 的實際行為**（容器裡的 LibreOffice 連純文字檔都載不進去）。
  測試驗的是「封裝是合法的 OOXML、關聯指得到檔案、圖是以 SVG 放進去的」。
  改動這一塊之後，一定要拿真的 Word 開一次並試「轉換成圖形」。

...
純靜態、純前端。主體是**七種填表產生器**（流程圖、泳道圖、組織圖、甘特圖、時間軸、
...
- `app/gen.js`：**七種產生器的純函式層**，外加「從 Excel 貼一整塊」與產生器的設定檔。
...
七個做圖入口的縮圖，是 `gen.build(gen.example, ...)` 真的畫出來的，不是範本、也不是圖片檔。
...
所以七種產生器全部是 Excel 做不到的那類；範本清單預設也只給 `COMMON_TYPES`——
